#[cfg(test)]
mod message_test{
    use rand::{thread_rng, RngCore};

    use crate::SecretData;
    static GF256_LOG: [u8; 256] = [
    ,
];
    static GF256_EXP: [u8; 256] = [
        ,
    ];
    fn gf256_sub(a: u8, b: u8) -> u8 {
        SecretData::gf256_add(a, b)
    }

    fn gf256_mul(a: u8, b: u8) -> u8 {
        if a == 0 || b == 0 {
            0
        } else {
            GF256_EXP[((u16::from(GF256_LOG[a as usize]) + u16::from(GF256_LOG[b as usize])) % 255)
                as usize]
        }
    }





    #[test]
    fn shamir_secret_scheme(){
        let secret_data = SecretData::with_secret("5", 3);
        let xishu=secret_data.coefficients.clone();
        //println!("{:?}",xishu);
        let share1 = secret_data.get_share(1).unwrap();
        println!("{:?}",share1);
        let share2 = secret_data.get_share(2).unwrap();
        println!("{:?}",share2);
        let share3 = secret_data.get_share(3).unwrap();
        println!("{:?}",share3);
        let share4=secret_data.get_share(4).unwrap();
        println!("{:?}",share4);
        println!("++++++++++++++++++++++++++++");
        //let recovered = SecretData::recover_secret(4, vec![share1,share2, share3,share4]).unwrap();
    
        //println!("Recovered :{}", recovered);
    }
    #[test]
    pub fn gf256_add()  {
        let vec="5".as_bytes();
        for c in vec {
            println!("{:?}",c )
        }
        println!("++++++++++++++++++++++++++++");
        let mut coefficients: Vec<Vec<u8>> = vec![];
        let mut rng = thread_rng();
        let mut rand_container = vec![0u8; (5 - 1) as usize];
        for c in "5".as_bytes() {
            println!("c={}",c);
            rng.fill_bytes(&mut rand_container);
            let mut coef: Vec<u8> = vec![*c];
            for r in rand_container.iter() {
                coef.push(*r);
            }
            //println!("{:?}",&coef );
            coefficients.push(coef);
        }
        println!("{:?}",coefficients );
        println!("++++++++++++++++++++++++++++");
        let secret_data = SecretData::with_secret("5 ", 5);
        println!("coefficients={:?},secret={:?}",secret_data.coefficients,secret_data.secret_data)
    }
    #[test]
    fn accumulate_share_bytes(){
        let id=2;
        let coefficient_bytes=[32, 9, 204, 199, 196];
        let mut accumulator: u8 = 0;

        let mut x_i: u8 = 1;

        for c in coefficient_bytes {
            accumulator = SecretData::gf256_add(accumulator, SecretData::gf256_mul(c, x_i));
            x_i = SecretData::gf256_mul(x_i, id);
        }
        println!("x_i={},accumulator={}",x_i,accumulator)
    }
    #[test]
    fn mul_test(){
        let mul=SecretData::gf256_mul(2, 4);
        println!("{}",mul)
    }
    #[test]
    fn get_share(){
        let id=2;
        let mut share_bytes: Vec<u8> = vec![];
        let coefficients = vec![vec![53, 226, 195, 30, 79]];//self.coefficients.clone();
        for coefficient in coefficients {
            let b = SecretData::accumulate_share_bytes(id, coefficient).unwrap();
            share_bytes.push(b);
        }

        share_bytes.insert(0, id);
        println!("{:?}",share_bytes)
    }
    #[test]
    fn vec_test(){
        let mut fxs=vec![];
        let share=vec![1,2,3,4];
        fxs.push(share[1..][0]);
        println!("{:?}",fxs)
    }
    #[test]
    fn test_langeran(){
        // [1, 187]
        // [2, 191]
        // [3, 49]
        // [4, 56]
        let xs: Vec<u8>=vec![2,3,4];
        let mut fxs: Vec<u8>=vec![193,55,68];//191,49,56
        let secret=SecretData::full_lagrange(&xs, &fxs);
        println!("{:?}",secret)
    }
}